...
Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui ratione voluptatem sequi nesciunt. Neque porro quisquam est, qui dolorem ipsum quia dolor sit amet, consectetur, adipisci velit.`;
  ...
  const splitTextContent = () => {
    ...
    // Créer un élément de test pour mesurer le texte
    ...
    // Hauteur disponible pour le texte (plus conservateur)
    const availableHeight = leftPage.offsetHeight - 280; // Augmenté de 250 à 280px pour plus de marge

    let leftText = '';
    let i = 0;

    // Ajouter les mots un par un jusqu'à approcher la limite
    while (i < words.length) {
      const testText = leftText + (leftText ? ' ' : '') + words[i];
      testElement.textContent = testText;
      
      // Vérifier si l'ajout de ce mot causerait un débordement
      if (testElement.offsetHeight > availableHeight) {
        // Si c'est le premier mot, on le garde quand même
        if (leftText === '') {
          leftText = words[i];
          i++;
        }
    ...
    // Le reste va sur la page de droite

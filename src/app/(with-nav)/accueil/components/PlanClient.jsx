...
export default function AccueilPlanInteractif({ salles }) {
  ...
  // Pré-tri + flags une seule fois
  const sortedAndFilteredSalles = useMemo(() => {
    const sorted = [...salles].sort((a, b) => a.displayOrder - b.displayOrder);
    return sorted.map((salle) => {
      const isGeneric = /^S\.\d+$/.test(salle?.name || "");
      return { ...salle, isGeneric, displayText: isGeneric ? "" : salle.name };
    });
  }, [salles]);
  ...
  const MobileRoomsList = () => (
    <div className="lg:hidden min-h-screen border-1 border-[var(--brown)] rounded-lg bg-gradient-to-b from-[var(--header-background)] to-[var(--login-background)] w-full overflow-x-hidden">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {sortedAndFilteredSalles.some((s) => s.status === "maintenance") && (
            ...
            {sortedAndFilteredSalles.map((salle) => {
              ...
              return (
                <button
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") handleRoomClick(e, slug);
                  }}
                >
                  {displayText && (
                    <span className="room-label text-center text-[#1b1610] px-2 py-1">
